...
>>>>>>> 14f3ba056b6ec54ff71052be620c08ba0d65c86c
...
<<<<<<< HEAD
   ...
   * @returns {Promise}
   */
  static requestJSON(url, gotOptions = { json: true }) {
  ...
  static itemDb(dbName, gotOptions = {}) {
    const url = `${ this.options.baseUrl }itemdb/${ dbName }`;
  ...
  static history(gotOptions = {}) {
    const url = `${ this.options.baseUrl }history/json/`;
  ...
  callMethodWithKey(method, gotOptions = {}) {
    const url = `${ this.apiUrl }/${ encodeURI(method) }/?key=${ this.options.apiKey }`;
   ...
   * @returns {Promise}
   */
  itemGetInfo(item, language = CSGOtmAPI.LANGUAGES.EN, gotOptions = {}) {
...
<<<<<<< HEAD
   ...
   *
   * @param {Array} items One item or array of items
   ...
   * @returns {Promise}
   */
  sellCreateTradeRequest(botId, type = CSGOtmAPI.CREATE_TRADE_REQUEST_TYPE.OUT, gotOptions = {}) {
    const url = `ItemRequest/${ type }/${ String(botId) }`;
  ...
  searchItemsByName(items, gotOptions = {}) {
    const url = `${ this.apiUrl }/MassSearchItemByName/key=${ this.options.apiKey }`;
    const list = [];
   ...
   * @returns {Promise}
   */
  searchItemByName({ market_hash_name }, gotOptions = {}) {
    const url = `SearchItemByName/${ market_hash_name }`;
